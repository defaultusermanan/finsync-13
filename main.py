...
def detect_languages_from_tree_entries(entries: List[Dict]) -> List[str]:
    found = []
    # build a joined path string for heuristic checks
    paths = " ".join(item.get("path", "").lower() for item in entries)
    def add(lang):
        if lang not in found:
            found.append(lang)
    # heuristics
    if "package.json" in paths or "tsconfig.json" in paths or ".js " in paths or ".jsx" in paths or ".ts " in paths or ".tsx" in paths:
        add("javascript-typescript")
    if "requirements.txt" in paths or "pyproject.toml" in paths or "setup.py" in paths or ".py " in paths:
        add("python")
    if "go.mod" in paths or re.search(r"\.go\b", paths):
        add("go")
    if "cargo.toml" in paths or re.search(r"\.rs\b", paths):
        add("rust")
    if "pom.xml" in paths or "build.gradle" in paths or re.search(r"\.java\b", paths) or re.search(r"\.kt\b", paths):
        add("java-kotlin")
    if re.search(r"\.swift\b", paths):
        add("swift")
    if "gemfile" in paths or re.search(r"\.rb\b", paths):
        add("ruby")
    if re.search(r"\.(c|cpp|h)\b", paths):
        add("c-cpp")
    if re.search(r"\.cs\b", paths):
        add("csharp")
    # actions is useful to include for workflow metadata
    add("actions")
    # fallback: prefer common languages if only actions found
    if found == ["actions"]:
        add("python")
    return found

def generate_codeql_workflow_yaml(languages: List[str], branch: str = "main", schedule_cron: str = "32 5 * * 2") -> str:
    """
    Generate a CodeQL Advanced workflow YAML that follows the layout and comments
    of the GitHub 'Create new workflow' reference the user provided.
    """
    # normalize languages and choose build-mode
    compiled = {"c-cpp", "java-kotlin", "go", "rust", "csharp", "swift"}
    if not languages:
        languages = ["actions", "python"]
    # Ensure actions included first (like the reference)
    if "actions" not in languages:
        languages.insert(0, "actions")
    # Build the include block (each entry under matrix.include)
    include_entries = []
    ...
    # Build full YAML (careful to escape f-string braces)
        ...
        if op_codeql:
            if not target_repo:
                ...
                if not detected_languages:
                    detected_languages = ["actions", "python"]
